...
pub struct Velocity<N: RealField = f32> {
    /// The linear velocity.
...
impl<N: RealField> Velocity<N> {
    ...
    pub fn angular(wx: N, wy: N, wz: N) -> Self {
        Velocity::new(na::zero(), Vector3::new(wx, wy, wz))
    }

    /// Create a purely linear velocity.
    #[inline]
    pub fn linear(vx: N, vy: N, vz: N) -> Self {
        Velocity::new(Vector3::new(vx, vy, vz), na::zero())
    }
...
impl<N: RealField> Add<Velocity<N>> for Velocity<N> {
    ...
    fn add(self, rhs: Self) -> Self {
        Velocity::new(self.linear + rhs.linear, self.angular + rhs.angular)
    }
}
...
impl<N: RealField> Mul<N> for Velocity<N> {
    ...
    fn mul(self, rhs: N) -> Self {
        Velocity::new(self.linear * rhs, self.angular * rhs)
    }
}